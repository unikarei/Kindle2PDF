...
rem ========================================================================
rem Git Commit Script (git_commit.bat)
...
if errorlevel 1 (
    ...
    echo Remote may have commits ahead (non-fast-forward).
    ...
    if /I "!FORCE_PUSH!"=="Y" (
        ...
        if errorlevel 1 (
            ...
            echo [Info] Alternative: run 'git pull --rebase origin %CUR_BRANCH%' then push again.
            pause
            exit /b 1
        ) else (
            echo.
        ...
        echo [Info] Cancelled. Alternative: run 'git pull --rebase origin %CUR_BRANCH%' then push again.
        pause
        exit /b 1
    )
)
