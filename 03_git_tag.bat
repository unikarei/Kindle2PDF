...
:found_tag
if "%LATEST_TAG%"=="" set LATEST_TAG=(none)
...
set /p VERSION="Enter NEW tag version (e.g., v1.0.0): "
if "%VERSION%"=="" (
...
echo [Success] Tag %VERSION% created locally.

rem [9] Check if remote 'origin' exists before pushing
git remote get-url origin >nul 2>nul
if errorlevel 1 (
    echo.
    echo [Info] Remote 'origin' is not configured. Skipping push.
    echo [Info] Tag created locally. To push later, run:
    echo        git push origin %VERSION%
    goto :done
)

rem [10] Push only the created tag to the remote (do not push branches)
echo.
